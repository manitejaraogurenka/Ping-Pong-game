...
function resetBoard(rodName) {
    ...
    if (rodName === rod2Name) {
        ...
        ballSpeedY = 2;
        ball.style.bottom = (rod1.offsetTop + rod1.offsetHeight) + 'px';
        ballSpeedY = 2;
...
window.addEventListener('keydown', function (event) {
    ...
    if ((event.key === 'ArrowRight' || event.code === 'KeyD') && ((rodRect.x + rodRect.width) < window.innerWidth)) {
        rod1.style.left = (rodRect.x) + rodSpeed + 'px';
        rod2.style.left = rod1.style.left;
    } else if ((event.key === 'ArrowLeft' || event.code === 'KeyA') && (rodRect.x > 0)) {
        rod1.style.left = (rodRect.x) - rodSpeed + 'px';
        rod2.style.left = rod1.style.left;
    }
